...
tbtf.plot_quadratic_fit(df_in_sample, n=10, state=10, ax=axes[0])
tbtf.plot_exponential_fit(df_in_sample, n=10, state=10, ax=axes[1])
fig.suptitle(f"Fitted Capital Share Functions at {in_end}")
plt.tight_layout()

fig, axes = plt.subplots(2, 2, figsize=(14, 10))
tbtf.plot_mean_fit(df_in_sample, n=10, state=10, ax=axes[0, 0])
tbtf.plot_loess_fit(df_in_sample, n=10, state=10, ax=axes[0, 1])
tbtf.plot_bayes_fit(df_in_sample, n=10, state=10, ax=axes[1, 0])
tbtf.plot_spline_fit(df_in_sample, n=10, state=10, ax=axes[1, 1])
...
To assess the robustness of nonparametric capital share estimation, we compare four smoothing methods, organized in paired subplots as follows:

| **Subplot Left** | **Subplot Right** | **Objective**                                      |
|------------------|-------------------|----------------------------------------------------|
| Mean Fit         | LOESS Fit         | Comparison of nonparametric, empirical smoothing   |
| Bayesian Fit     | Spline Fit        | Comparison of statistically regularized smoothers  |

We begin with the first pair: **mean-based fitting** and **LOESS fitting**.
