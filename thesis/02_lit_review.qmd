...
- The Capital Asset Pricing Model (CAPM) asserts that expected excess returns are proportional to market beta [@FamaMacBeth1973].
...
- The Betting Against Beta (BAB) framework suggests that low-beta assets outperform on a risk-adjusted basis [@FrazziniPedersen2014].
- Mean-Variance Optimization, introduced by Markowitz (1952), assumes that returns are normally distributed and fully explained by their mean and covariance structure [@Markowitz1952].
...
### 2.1 Industry-Level CAPM Failure  
- Single-Factor CAPM (Fama-MacBeth Regression): When applied to industry portfolios (SIC 10), it reveals large, persistent unexplained alphas [@FamaFrench1997]. Even in high-tech sectors, beta fails to fully account for cross-sectional return variation.
- Two-Factor Model (MKT + SMB): Using FF 49 industry portfolios, the market risk premium becomes statistically insignificant post-2010. The size effect appears reversed, with small-cap underperformance contradicting theoretical expectations. However, Asness et al. [@asness2018size] argue that the size premium persists when controlling for quality factors, suggesting the reversal may stem from low-quality small-cap stocks. Additionally, Hou et al. [@hou2020replicating] find that many documented anomalies, including size-related ones, do not hold up under rigorous testing, particularly when mitigating microcap stocks and using value-weighted returns.

### 2.2 Linear Factor Models and Multicollinearity  
- Excess returns of FF 10 industry portfolios are regressed on macro and financial factors, revealing multicollinearity among predictors like market excess returns, value spreads, and volatility indicators, leading to unstable coefficients and weak out-of-sample explanatory power.  
...
### 2.3 Betting Against Beta and Sharpe Ratio Symmetry  
...
### 2.4 Practical Failures of Mean-Variance Optimization  
...
### 2.5 Limitations of Bayesian Optimization and Tilting  
- @black1992global combines CAPM equilibrium with investor views via Bayesian updating.  
- @brandt2009parametric incorporate momentum and size signals into dynamic allocation, but empirical tests reveal no consistent outperformance over ETF benchmarks post-2010, suffering from estimation error and overfitting in regimes characterized by unstable return distributions.
...
- Large-cap firms are insulated from downward transitions, mirroring the 'superstar' effect where small quality differences yield large market outcomes, as described by Rosen [@rosen1981economics]. In banking, Gandhi and Lustig [@gandhi2015size] find large banks have lower risk-adjusted returns, indicative of implicit government guarantees in adverse scenarios.  
...
- Return distributions reflect arbitrage-limited capital flows, ETF-induced inflows, and policy distortions, with Jiang, Vayanos, and Zheng [@jiang2020passive] showing passive investing disproportionately benefits large firms, exacerbating concentration. This aligns with Autor et al. [@autor2020fall], linking market power concentration to declining labor share, highlighting structural market dynamics.
...
This study extends literature on asset pricing anomalies, building on Hou et al.  regarding anomaly robustness. It introduces Markov-based transition models for capital lock-in, aligning with rising concentration insights from Autor et al.  and Jiang et al. . It offers low-turnover, high-performance portfolio strategies and critiques ETF-driven distortions and post-QE concentration, reflecting real-world finance breakdowns under structural stress.
